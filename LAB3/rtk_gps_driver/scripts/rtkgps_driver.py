...
import rospy
import serial
from rtk_gps_driver.msg import RTKGPS
import utm
import sys
...
def getGNSSData(gns_values):
    ...
    gnss_raw_data['quality'] = gnss_info[6]
...
def publishCustomMessage(gps_data, utm_data):
    ...
    gns_message = RTKGPS()
    gns_message.header.frame_id = "RTKGPS message"
    ...
    gns_message.quality = gps_data['quality']
    gns_publisher = rospy.Publisher('gps'+'/rtk_gps',RTKGPS, queue_size=5)
    ...
    try:
        while not rospy.is_shutdown():
            ...
            if gnss_data is None:
                ...
                if gnss_data.startswith('$GNGGA'):
                    
                    rospy.loginfo("Received GNGGA data!!")
                    seq = 0
